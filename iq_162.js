...
function preventImpossible() {
  ...
  if (Number($("#DIST-ID").val()) < (.38*conversion)) {
    $("#DIST-ID, #DIST-ID-RANGE").val(.38*conversion);
    return false;
  }
  if (Number($("#PROX-ID").val())  < (.38*conversion)) {
    $("#PROX-ID, #PROX-ID-RANGE").val(.38*conversion);
    return false;
  }
...
//START BALLOON VALIDATION
    ...
    function checkDistID(compliant, conversion){
      ...
      var singleWall = Number($("#DWT").val()) / 2;
      if(compliant) {
        if(DIST_WT >= (singleWall*6) && DIST_WT <= (singleWall*8)) { $("#DIST-ID").removeClass('error'); return true; } else { $("#DIST-ID").addClass('error'); return false; }
      } else {
        if(DIST_WT >= (singleWall*4) && DIST_WT <= (singleWall*5)) { $("#DIST-ID").removeClass('error'); return true; } else { $("#DIST-ID").addClass('error'); return false; }
      }  
    }
    ...
    function checkProxID(compliant, conversion){
      ...
      var singleWall = Number($("#DWT").val()) / 2;
      if(compliant) {
        if(PROX_WT >= (singleWall*6) && PROX_WT <= (singleWall*8)) { $("#PROX-ID").removeClass('error'); return true; } else { $("#PROX-ID").addClass('error'); return false; }
      } else {
        if(PROX_WT >= (singleWall*4) && PROX_WT <= (singleWall*5)) { $("#PROX-ID").removeClass('error'); return true; } else { $("#PROX-ID").addClass('error'); return false; }
      }  
    }
...
$("#DIST-OD-RANGE").on('input', function() {
  if(this.value - Number($("#DWT").val()) < Number($("#DIST-ID").val())) {
    $("#DIST-ID-RANGE").val(this.value - Number($("#DWT").val())).trigger("input");
  }
});
$("#PROX-OD-RANGE").on('input', function() {
  if(this.value - Number($("#DWT").val()) < Number($("#PROX-ID").val())) {
    $("#PROX-ID-RANGE").val(this.value - Number($("#DWT").val())).trigger("input");
  }
});
$("#DIST-ID-RANGE").on('input', function() {
  if(this.value + Number($("#DWT").val()) > Number($("#DIST-OD").val())) {
    $("#DIST-OD-RANGE").val(this.value + Number($("#DWT").val())).trigger("input");
  }
});
$("#PROX-ID-RANGE").on('input', function() {
  if(this.value + Number($("#DWT").val()) > Number($("#PROX-OD").val())) {
    $("#PROX-OD-RANGE").val(this.value + Number($("#DWT").val())).trigger("input");
  }
});
...
<div class="row">
    ...
    <div class="item"><p class="label">Dist ID</p><input class="quote-input" id="l`+lineItem+`-DIST-ID" value="`+DIST_ID+`" readonly></input></div>
    ...
    <div class="item"><p class="label">Prox ID</p><input class="quote-input" id="l`+lineItem+`-PROX-ID" value="`+PROX_ID+`" readonly></input></div>
...
$("#DIST-ID, #DIST-ID-RANGE").val($("#l"+ line + "-DIST-ID").val());
...
$("#PROX-ID, #PROX-ID-RANGE").val($("#l"+ line + "-PROX-ID").val());
