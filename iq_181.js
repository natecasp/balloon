...
function changeThickness( WT ) {
  if(['Vestamid ML21','PET','Pebax 55D', 'Pebax 63D', 'Pebax 72D', ''].includes($("#MATERIAL").val())) {
   ...
   if($("#SUGGESTED-DIST-ID").is(":checked")) {
         if(compliant) {
           $("#DIST-ID-RANGE, #DIST-ID").val(Number($("#DIST-OD").val())-((params.WT*4)*2));
           var distTailWT = params.WT*4;
         } else {
          $("#DIST-ID-RANGE, #DIST-ID").val(Number($("#DIST-OD").val())-((params.WT*2)*2));
     ...
     if($("#SUGGESTED-PROX-ID").is(":checked")) {
         if(compliant) {
           $("#PROX-ID-RANGE, #PROX-ID").val(Number($("#PROX-OD").val())-((params.WT*4)*2));
           var proxTailWT = params.WT*4;
         } else {
          $("#PROX-ID-RANGE, #PROX-ID").val(Number($("#PROX-OD").val())-((params.WT*2)*2));
