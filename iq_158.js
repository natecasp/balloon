...
//START BALLOON VALIDATION
    ...
    function checkLength(compliant, conversion){
      ...
      var LEN = Number($("#LEN").val()) + Number($("#DIST-NECK").val()) + Number($("#PROX-NECK").val()) + parseFloat(distLengthNumber) + parseFloat(proxLengthNumber)
    ...
    function checkDistIDTol(compliant, conversion){
      ...
      if(compliant) {
        if(DIST_ID_TOL >= (0.0254*conversion)) { $("#DIST-ID-TOL").removeClass('error'); return true; } else { $("#DIST-ID-TOL").addClass('error'); return false; }
      } else {
        if(DIST_ID_TOL >= (0.0254*conversion)) { $("#DIST-ID-TOL").removeClass('error'); return true; } else { $("#DIST-ID-TOL").addClass('error'); return false; }
      }  
    }
    ...
    function checkProxIDTol(compliant, conversion){
      ...
      if(compliant) {
        if(PROX_ID_TOL >= (0.0254*conversion)) { $("#PROX-ID-TOL").removeClass('error'); return true; } else { $("#PROX-ID-TOL").addClass('error'); return false; }
      } else {
        if(PROX_ID_TOL >= (0.0254*conversion)) { $("#PROX-ID-TOL").removeClass('error'); return true; } else { $("#PROX-ID-TOL").addClass('error'); return false; }
      } 
    }
