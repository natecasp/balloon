...
function preventImpossible() {
  ...
  if (Number($("#DIST-OD").val()) - (Number($("#DIST-WT").val())*2) < (.38*conversion)) {
    $("#DIST-WT, #DIST-WT-RANGE").val(($("#DIST-OD").val() - (.38*conversion))/2);
    return false;
  }
  if (Number($("#PROX-OD").val()) - (Number($("#PROX-WT").val())*2) < (.38*conversion)) {
    $("#PROX-WT, #PROX-WT-RANGE").val(($("#PROX-OD").val() - (.38*conversion))/2);
    return false;
  }
  return true;
}
...
path.arc(params.radius*conversion, 0, params.radius, Math.PI, Math.PI/2,true);
...
path.arc(0, params.radius*conversion, -params.radius, Math.PI/2, 0,true);
}
//Distal Tail
...
//START BALLOON VALIDATION
    ...
    function checkDistNeck(compliant, conversion){
      ...
      if(compliant) {
        if(DIST_NECK >= (2*conversion) && DIST_NECK <= (25*conversion)) { $("#DIST-NECK").removeClass('error'); $("#dist-neck-dialog").dialog("close"); return true; } else { $("#DIST-NECK").addClass('error'); $("#dist-neck-dialog").dialog("open");  return false; }
      } else {
        if(DIST_NECK >= (2*conversion) && DIST_NECK <= (25*conversion)) { $("#DIST-NECK").removeClass('error'); $("#dist-neck-dialog").dialog("close"); return true; } else { $("#DIST-NECK").addClass('error'); $("#dist-neck-dialog").dialog("open"); return false; }
      }
    }
    ...
    function checkLengthTol(compliant, conversion){
      ...
      if(compliant) {
        if(LEN_TOL >= (1*conversion)) { $("#LEN-TOL").removeClass('error'); return true; } else { $("#LEN-TOL").addClass('error'); return false; }
      } else {
        if(LEN_TOL >= (1*conversion)) { $("#LEN-TOL").removeClass('error'); return true; } else { $("#LEN-TOL").addClass('error'); return false; }
      }    
    }
    ...
    function checkDWT(compliant, conversion){
      ...
      if(compliant) {
        if(DWT >= (0.0381*conversion) && DWT <= (0.6096*conversion)) { $("#DWT").removeClass('error'); $("#dwt-dialog").dialog("close"); return true; } else { $("#DWT").addClass('error'); $("#dwt-dialog").dialog("open"); return false; }
      } else {
        if(DWT >= (0.0381*conversion) && DWT <= (0.6096*conversion)) { $("#DWT").removeClass('error'); $("#dwt-dialog").dialog("close"); return true; } else { $("#DWT").addClass('error'); $("#dwt-dialog").dialog("open"); return false; }
      }  
    }
    function checkDWTTol(compliant, conversion){
      ...
      if(compliant) {
        if(DWT_TOL >= (0.0127*conversion)) { $("#DWT-TOL").removeClass('error'); return true; } else { $("#DWT-TOL").addClass('error'); return false; }
      } else {
        if(DWT_TOL >= (0.0127*conversion)) { $("#DWT-TOL").removeClass('error'); return true; } else { $("#DWT-TOL").addClass('error'); return false; }
      }  
    }
    ...
    function checkDistOdTol(compliant, conversion){
      ...
      if(compliant) {
        if(DIST_OD_TOL >= (0.0762*conversion)) { $("#DIST-OD-TOL").removeClass('error'); return true; } else { $("#DIST-OD-TOL").addClass('error'); return false; }
      } else {
        if(DIST_OD_TOL >= (0.0762*conversion)) { $("#DIST-OD-TOL").removeClass('error'); return true; } else { $("#DIST-OD-TOL").addClass('error'); return false; }
      }  
    }
    ...
    function checkProxOdTol(compliant, conversion){
      ...
      if(compliant) {
        if(PROX_OD_TOL >= (0.0762*conversion)) { $("#PROX-OD-TOL").removeClass('error'); return true; } else { $("#PROX-OD-TOL").addClass('error'); return false; }
      } else {
        if(PROX_OD_TOL >= (0.0762*conversion)) { $("#PROX-OD-TOL").removeClass('error'); return true; } else { $("#PROX-OD-TOL").addClass('error'); return false; }
      }  
    }
    function checkDistWT(compliant, conversion){
      var DIST_WT = Number($("#DIST-WT").val());
      if(compliant) {
        if(DIST_WT >= (0.0254*conversion) && DIST_WT <= (0.762*conversion)) { $("#DIST-WT").removeClass('error'); return true; } else { $("#DIST-WT").addClass('error'); return false; }
      } else {
        if(DIST_WT >= (0.0254*conversion) && DIST_WT <= (0.762*conversion)) { $("#DIST-WT").removeClass('error'); return true; } else { $("#DIST-WT").addClass('error'); return false; }
      }  
    }
    function checkDistWTTol(compliant, conversion){
      var DIST_WT_TOL = Number($("#DIST-WT-TOL").val());
      if(compliant) {
        if(DIST_WT_TOL >= (0.0254*conversion)) { $("#DIST-WT-TOL").removeClass('error'); return true; } else { $("#DIST-WT-TOL").addClass('error'); return false; }
      } else {
        if(DIST_WT_TOL >= (0.0254*conversion)) { $("#DIST-WT-TOL").removeClass('error'); return true; } else { $("#DIST-WT-TOL").addClass('error'); return false; }
      }  
    }
    function checkProxWT(compliant, conversion){
      var PROX_WT = Number($("#PROX-WT").val());
      if(compliant) {
        if(PROX_WT >= (0.0254*conversion) && PROX_WT <= (0.762*conversion)) { $("#PROX-WT").removeClass('error'); return true; } else { $("#PROX-WT").addClass('error'); return false; }
      } else {
        if(PROX_WT >= (0.0254*conversion) && PROX_WT <= (0.762*conversion)) { $("#PROX-WT").removeClass('error'); return true; } else { $("#PROX-WT").addClass('error'); return false; }
      }  
    }
    function checkProxWTTol(compliant, conversion){
      var PROX_WT_TOL = Number($("#PROX-WT-TOL").val());
      if(compliant) {
        if(PROX_WT_TOL >= (0.0254*conversion)) { $("#PROX-WT-TOL").removeClass('error'); return true; } else { $("#PROX-WT-TOL").addClass('error'); return false; }
      } else {
        if(PROX_WT_TOL >= (0.0254*conversion)) { $("#PROX-WT-TOL").removeClass('error'); return true; } else { $("#PROX-WT-TOL").addClass('error'); return false; }
      } 
    }
...
function calculate() {
  ...
  if(['Vestamid ML21','PET','Pebax 55D', 'Pebax 63D', 'Pebax 72D', ''].includes($("#MATERIAL").val())) {
    ...
    var compliant = true;
      ...
      var f15 = checkDistWT(compliant, conversion)
      var f16 = checkDistWTTol(compliant, conversion)
      var f17 = checkProxWT(compliant, conversion)
      var f18 = checkProxWTTol(compliant, conversion)
...
$("#unit").on('change', function() {
  ...
  $("#DIST-WT, #DIST-WT-RANGE, #PROX-WT, #PROX-WT-RANGE").attr({
    "min": 0.002,
    "max": 0.03,
    "step": 0.001
  });
  $("#DIST-WT-TOL, #PROX-WT-TOL").attr({
    "min": 0,
    "step": 0.001
  });
