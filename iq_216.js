...
distalFolder.add( params, 'distConeAng', 3, 60, 1).name('Distal Cone Angle').onChange( createShape );
...
proximalFolder.add( params, 'proxConeAng', 3, 60, 1).name('Proximal Cone Angle').onChange( createShape );
...
//START BALLOON VALIDATION
      ...
      if(compliant) {
        if(DIST_NECK >= (2*conversion) && DIST_NECK <= (25*conversion)) {
          ...
          addError(
            ...
            'Increase distal neck length, or continue with custom quote',
            null,
            '#DIST-NECK'
        );
          }
          ...
          addError(
            ...
            'Decrease distal neck length, or continue with custom quote',
            null,
            '#DIST-NECK'
        );
          }
  ...
  if(compliant) {
    if(PROX_NECK >= (2*conversion) && PROX_NECK <= (25*conversion)) {
      removeError('prox-neck-small');
      removeError('prox-neck-large');
      return true;
    } else {
      if(PROX_NECK < (2*conversion)) {
        addError(
          'prox-neck-small',
          'Your proximal neck length is too small to qualify for an instant quote.',
          'Increase proximal neck length, or continue with custom quote',
          null,
          '#PROX-NECK'
        );
      }
      if(PROX_NECK > (25*conversion)) {
        addError(
          'prox-neck-large',
          'Your proximal neck length is too large to qualify for an instant quote.',
          'Decrease proximal neck length, or continue with custom quote',
          null,
          '#PROX-NECK'
        );
      }
...
function checkDistNeckTol(compliant, conversion){
  ...
  if(compliant) {
    if(DIST_NECK_TOL == 'min') {
      removeError('dist-neck-tol-invalid');
      return true;
    } else {
      addError(
        'dist-neck-tol-invalid',
        'Distal neck tolerance must be MIN to qualify for an instant quote',
        'Set tolerance to MIN, or continue with custom quote',
        null,
        '#DIST-NECK-TOL'
      );
      return false;
...
function checkProxNeckTol(compliant, conversion){
  ...
  if(compliant) {
    if(PROX_NECK_TOL == 'min') {
      removeError('prox-neck-tol-invalid');
      return true;
    } else {
      addError(
        'prox-neck-tol-invalid',
        'Proximal neck tolerance must be MIN to qualify for an instant quote',
        'Set tolerance to MIN, or continue with custom quote',
        null,
        '#PROX-NECK-TOL'
      );
...
function checkLength(compliant, conversion){
  ...
  if(LEN >= 0 && LEN <= maxLength) {
    ...
    if(LEN < 0) {
      addError(
        'length-too-short',
        'Total length cannot be negative',
        'Increase length parameters, or continue with custom quote',
        null,
        '#LEN'
      );
    }
    if(LEN > maxLength) {
      addError(
        'length-too-long',
        'Overall length is too large at this diameter for an instant quote',
        'Adjust length, cone angles, diameter, or continue with custom quote',
        null,
        '#LEN'
      );
    }
...
function checkLengthTol(compliant, conversion){
  ...
  if(compliant) {
    if(LEN_TOL >= (0.5*conversion)) {
      removeError('length-tol-small');
      return true;
    } else {
      addError(
        'length-tol-small',
        'Length tolerance is too small to qualify for an instant quote',
        'Increase length tolerance, or continue with custom quote',
        null,
        '#LEN-TOL'
      );
      return false;
...
function checkDiameter(compliant, conversion){
  var DIAMETER = Number($("#DIAMETER").val());
  if(compliant) {
    if(DIAMETER >= (1*conversion) && DIAMETER <= (33*conversion)) {
      removeError('diameter-out-of-range');
      return true;
    } else {
      if(DIAMETER < (1*conversion)) {
      addError(
        'diameter-out-of-range',
        'Diameter too small to qualify for an instant quote',
        'Increase diameter, or continue with custom quote',
        null,
        '#DIAMETER'
      );
      }
            if(DIAMETER > (33*conversion)) {
      addError(
        'diameter-out-of-range',
        'Diameter too large to qualify for an instant quote',
        'Decrease diameter, or continue with custom quote',
        null,
        '#DIAMETER'
      );
      }
...
function checkDiameterTol(compliant, conversion){
  ...
  if(compliant) {
    if(DIAMETER_TOL.toLowerCase() == 'min') {
      removeError('diameter-tol-invalid');
      return true;
    } else {
      addError(
        'diameter-tol-invalid',
        'Diameter tolerance must be MIN for compliant materials to qualify for an instant quote',
        'Set diameter tolerance to MIN, or continue with custom quote',
        null,
        '#DIAMETER-TOL'
      );
      return false;
    ...
    if(Number(DIAMETER_TOL) >= (0.5*conversion)) {
      removeError('diameter-tol-small');
      return true;
    } else {
      addError(
        'diameter-tol-small',
        'Diameter tolerance is too small to qualify for an instant quote',
        'Increase diameter tolerance, or continue with custom quote',
        null,
        '#DIAMETER-TOL'
      );
      return false;
...
function checkDWT(compliant, conversion){
  ...
  if(compliant) {
    if(DWT >= (0.05*conversion) && DWT <= (0.38*conversion)) {
      removeError('dwt-out-of-range');
      return true;
    } else {
      if (DWT < (0.05*conversion)) {
      addError(
        'dwt-out-of-range',
        'DWT is too small to qualify for an instant quote',
        'Increase DWT, or continue with custom quote',
        null,
        '#DWT'
      );
      }
      if (DWT > (0.38*conversion)) {
      addError(
        'dwt-out-of-range',
        'DWT is too large to qualify for an instant quote',
        'Decrease DWT, or continue with custom quote',
        null,
        '#DWT'
      );
      }
    ...
    if(DWT >= (0.05*conversion) && DWT <= (0.25*conversion)) {
      removeError('dwt-out-of-range');
      return true;
    } else {
      if (DWT < (0.05*conversion)) {
...
function checkDWTTol(compliant, conversion){
  ...
  if(compliant) {
    if(DWT_TOL >= (0.0125*conversion)) {
      removeError('dwt-tol-small');
      return true;
    } else {
      addError(
        'dwt-tol-small',
        'DWT tolerance is too small to qualify for an instant quote',
        'Increase DWT tolerance, or continue with custom quote',
        null,
        '#DWT-TOL'
      );
      return false;
...
function checkDistOdTol(compliant, conversion){
  ...
  if(compliant) {
    if(DIST_OD_TOL >= (0.075*conversion)) {
      removeError('dist-od-tol-small');
      return true;
    } else {
      addError(
        'dist-od-tol-small',
        'Distal OD tolerance is too small to qualify for an instant quote',
        'Increase distal OD tolerance, or continue with custom quote',
        null,
        '#DIST-OD-TOL'
      );
      return false;
...
function checkProxOdTol(compliant, conversion){
  ...
  if(compliant) {
    if(PROX_OD_TOL >= (0.075*conversion)) {
      removeError('prox-od-tol-small');
      return true;
    } else {
      addError(
        'prox-od-tol-small',
        'Proximal OD tolerance is too small to qualify for an instant quote',
        'Increase proximal OD tolerance, or continue with custom quote',
        null,
        '#PROX-OD-TOL'
      );
      return false;
...
function checkDistID(compliant, conversion){
  ...
  if(compliant) {
    if(DIST_WT <= (DWT*10)) {
      removeError('dist-id-wall-thick');
      return true;
    } else {
      addError(
        'dist-id-wall-thick',
        'Distal wall thickness is too thick for an instant quote',
        'Adjust distal ID or OD values, or continue with custom quote',
        null,
        '#DIST-ID'
      );
      return false;
...
function checkDistIDTol(compliant, conversion){
  ...
  if(compliant) {
    if(DIST_ID_TOL >= (0.025*conversion)) {
      removeError('dist-id-tol-small');
      return true;
    } else {
      addError(
        'dist-id-tol-small',
        'Distal ID tolerance is too small to qualify for an instant quote',
        'Increase distal ID tolerance, or continue with custom quote',
        null,
        '#DIST-ID-TOL'
      );
      return false;
...
function checkProxID(compliant, conversion){
  ...
  if(compliant) {
    if(PROX_WT <= (DWT*10)) {
      removeError('prox-id-wall-thick');
      return true;
    } else {
      addError(
        'prox-id-wall-thick',
        'Proximal wall thickness is too thick for an instant quote',
        'Adjust proximal ID or OD values, or continue with custom quote',
        null,
        '#PROX-ID'
      );
      return false;
...
function checkProxIDTol(compliant, conversion){
  ...
  if(compliant) {
    if(PROX_ID_TOL >= (0.025*conversion)) {
      removeError('prox-id-tol-small');
      return true;
    } else {
      addError(
        'prox-id-tol-small',
        'Proximal ID tolerance is too small to qualify for an instant quote',
        'Increase proximal ID tolerance, or continue with custom quote',
        null,
        '#PROX-ID-TOL'
      );
      return false;
...
function checkDistCone(compliant, conversion){
  if (!$("#DIST-CURVE").is(":checked")) {
    ...
    if(DIST_CONE >= 20 && DIST_CONE <= 60) {
      removeError('dist-cone-out-of-range');
      return true;
    } else {
      if (DIST_CONE < 20){
      addError(
        'dist-cone-out-of-range',
        'Distal cone angle is too small to qualify for an instant quote',
        'Increase distal cone angle, or continue with custom quote',
        null,
        '#DIST-CONE'
      );
    }
      ...
      addError(
        'dist-cone-out-of-range',
        'Distal cone angle is too large to qualify for an instant quote',
        'Decrease distal cone angle, or continue with custom quote',
        null,
        '#DIST-CONE'
      );
    }
...
function checkDistConeTol(compliant, conversion){
  if (!$("#DIST-CURVE").is(":checked")) {
    ...
    if(DIST_CONE_TOL >= 3) {
      removeError('dist-cone-tol-small');
      return true;
    } else {
      addError(
        'dist-cone-tol-small',
        'Distal cone tolerance is too small to qualify for an instant quote',
        'Increase cone tolerance, or continue with custom quote',
        null,
        '#DIST-CONE-TOL'
      );
      return false;
...
function checkProxCone(compliant, conversion){
  if (!$("#PROX-CURVE").is(":checked")) {
    ...
    if(PROX_CONE >= 20 && PROX_CONE <= 60) {
      removeError('prox-cone-out-of-range');
      return true;
    } else {
      if (PROX_CONE < 20) {
      addError(
        'prox-cone-out-of-range',
        'Proximal cone angle is too small to qualify for an instant quote',
        'Increase proximal cone angle, or continue with custom quote',
        null,
        '#PROX-CONE'
      );
    }
      ...
      addError(
        'prox-cone-out-of-range',
        'Proximal cone angle is too large to qualify for an instant quote',
        'Decrease proximal cone angle, or continue with custom quote',
        null,
        '#PROX-CONE'
      );
    }
...
function checkProxConeTol(compliant, conversion){
  if (!$("#PROX-CURVE").is(":checked")) {
    ...
    if(PROX_CONE_TOL >= 3) {
      removeError('prox-cone-tol-small');
      return true;
    } else {
      addError(
        'prox-cone-tol-small',
        'Proximal cone tolerance is too small to qualify for an instant quote',
        'Increase cone tolerance, or continue with custom quote',
        null,
        '#PROX-CONE-TOL'
      );
...
function checkHoop(compliant) {
  ...
  var DIAMETER = Number($("#DIAMETER").val()); 
  var distHoop = DIAMETER / distID;
  var proxHoop = DIAMETER / proxID;
  //var hoopRatio = DIAMETER / extrusionID;
  var maxHoop = $("#MATERIAL").val() == 'Vestamid ML21' ? 6 : 8;
  if ( (distHoop > maxHoop) || (proxHoop > maxHoop) ) {
  if (distHoop > maxHoop) {
    addError(
        'dist-hoop-ratio-high',
        'Distal hoop ratio is too high to qualify for an instant quote',
        'Adjust diameter or distal ID, or continue with custom quote',
        null,
        '#DIST-ID'
      );
  } else {
    removeError('dist-hoop-ratio-high');
  }

  if (proxHoop > maxHoop) {
    addError(
        'prox-hoop-ratio-high',
        'Proximal hoop ratio is too high to qualify for an instant quote',
        'Adjust diameter or proximal ID, or continue with custom quote',
        null,
        '#PROX-ID'
      );
  } else {
    removeError('prox-hoop-ratio-high');
  }
    return false;
  } else {
    removeError('dist-hoop-ratio-high');
...
function checkFatigue(compliant) {
  if(compliant) {
    removeError('fatigue-too-high');
    return true;
  } else {
    if(Number($("#REQUESTED-FATIGUE").val()) > 10) {
      addError(
        'fatigue-too-high',
        'Requested fatigue is too high to qualify for an instant quote',
        'Reduce fatigue value, or continue with custom quote',
        null,
        '#REQUESTED-FATIGUE'
      );
      return false;
...
if(this.checked){
  ...
  if($('.line-greenlight:checkbox:checked').length == 0) {
    $("#add-extrusion").text("Add to Instant Quote");
  }
  $("#custom-dialogue").css('display', 'none');
} else {
  $("#price-block").css('display', 'none');
  $("#add-extrusion").text("Add to Custom Quote");
}
...
$("#dist-wt-notice").on({
    ...
    mouseleave: function () {
      ...
      $("#dist-wt-details-dialog").dialog("close");
    }
});
$("#prox-wt-notice").on({
    mouseenter: function () {
    ...
    mouseleave: function () {
        //stuff to do on mouse leave
      $("#prox-wt-details-dialog").dialog("close");
    }
});
...
if($("#MATERIAL").val() == '') {
  $("#MATERIAL").css('border', '3px solid #ff6f6f');$("#MATERIAL").get(0).scrollIntoView(); $("#greenlight").prop('checked', false); $("greenlight").trigger('change');  
  addError(
        'material',
        'Select a material before adding this balloon to your quote',
        'Select a material',
        null,
        '#MATERIAL'
      );
...
$("#continue-to-checkout").removeClass('hide-price');
  ...
  $("#add-extrusion").css('display', 'none');
  } 
...
$("#review").on("click", function() {
  $("#custom-dialogue").css('display', 'none');
  $("#add-extrusion").css('display', 'block');
});
...
`<div class="line" id="line`+lineItem+`" style="display:none;">
  	...
  	<input class="quote-input price-item" id="l`+lineItem+`-price" value="`+item.price+`" readonly></input>
    <input class="linenumber" id="l`+lineItem+`-line" value="`+lineItem+`" readonly style="display:none;"></input>
</div>`;
$("#quote").append(lineHtml);
