...
var distAngle = ((Number($("#diameter-range").val())-Number($("#distalid-range").val()))/2) * (Math.sin((90 - Math.min(Number($("#distalcone-range").val()),89.999999)) *(Math.PI/180)) / Math.sin(Number($("#distalcone-range").val()) *(Math.PI/180)));
var proxAngle = ((Number($("#diameter-range").val())-Number($("#proximalid-range").val()))/2) * (Math.sin((90 - Math.min(Number($("#proximalcone-range").val()),89.999999)) *(Math.PI/180)) / Math.sin(Number($("#proximalcone-range").val()) *(Math.PI/180)));
console.log('diameter-proximalid divided by two:' + ((Number($("#diameter-range").val())-Number($("#proximalid-range").val()))/2));
console.log('proximal cone angle: '+Number($("#proximalcone-range").val()));
console.log('other angle:'+ (90-(Math.min(Number($("#proximalcone-range").val()), 89.99999))));
console.log('first sin:' + Math.sin(90 - (Math.min(Number($("#proximalcone-range").val()),89.999999) *(Math.PI/180))))
console.log(proxAngle);
if((Number($("#len-range").val())+distAngle+proxAngle) > maxLength) {
var newBody = Math.max((maxLength - distAngle - proxAngle), 0).toFixed(1);
$("#LEN,#len-range").attr("max", newBody);
$("#LEN,#len-range").val(newBody);
...
$("#spherical").on('change', function() {
  ...
  distalCurveController.setValue(true);
  proximalCurveController.setValue(true);
  lengthController.setValue(0);
  $("#len-range,#LEN").val(0);
  $("#distal-round,#proximal-round").prop('checked', true);
  } 
  else {distalCurveController.setValue(false);
  distalCurveController.setValue(false);
  proximalCurveController.setValue(false);
  lengthController.setValue(2);
  $("#len-range,#LEN").val(2);
  $("#distal-round,#proximal-round").prop('checked', false);
  }
...
var submit = false;
$("#wf-form-Balloon-Designer").on('submit', function(e) {
...
setTimeout(function() {
  ...
  //return true;
  submit = true;
  $("#wf-form-Balloon-Designer").submit();
}, 2000);
