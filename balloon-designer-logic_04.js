...
$("#unit-2").on('change', function() {
  if(this.value == 'in') {
    ...
    $("#LEN,#len-range").attr({
      "min": Number($(this).attr("min"))/25.4,
      "max": Number($(this).attr("max"))/25.4,
      "step": Number($(this).attr("step"))/25.4
    });
    console.log($("#len-range").attr("min"));
    ...
    //DIAMETER
    var newDiameter = $("#diameter-range").val() / 25.4;
    $("#DIAMETER,#diameter-range").attr({
      "min": $(this).attr("min")/25.4,
      "max": $(this).attr("max")/25.4,
      "step": $(this).attr("step")/25.4
    });
    $("#DIAMETER,#diameter-range").val(newDiameter);
    diameterController.setValue(newDiameter);

    //DISTAL ID
    var newDistalId = $("#distalid-range").val() / 25.4;
    ...
    //PROXIMAL ID
    var newProximalId = $("#proximalid-range").val() / 25.4;
    ...
    //PROXIMAL NECK
    var newProximalNeck = $("#proximalneck-range").val() / 25.4;
    ...
    //DISTAL NECK
    var newDistalNeck = $("#distalneck-range").val() / 25.4;
