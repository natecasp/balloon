...
//START BALLOON VALIDATION
    ...
    function checkDistNeck(compliant){
      ...
      if(compliant) {
        if(DIST_NECK >= 2 && DIST_NECK <= 25) { $("#DIST-NECK").removeClass('error'); return true; } else { $("#DIST-NECK").addClass('error'); return false; }
      } else {
        if(DIST_NECK >= 2 && DIST_NECK <= 25) { $("#DIST-NECK").removeClass('error'); return true; } else { $("#DIST-NECK").addClass('error'); return false; }
      }
    }
    function checkProxNeck(compliant){
      ...
      if(compliant) {
        if(PROX_NECK >= 2 && PROX_NECK <= 25) { $("#PROX-NECK").removeClass('error'); return true; } else { $("#PROX-NECK").addClass('error'); return false; }
      } else {
        if(PROX_NECK >= 2 && PROX_NECK <= 25) { $("#PROX-NECK").removeClass('error'); return true; } else { $("#PROX-NECK").addClass('error'); return false; }
      }  
    }
    ...
    function checkLength(compliant){
      ...
      if($("#SPHERICAL").is(':checked') && LEN > 0) {
        $("#SPHERICAL").prop('checked', false);
      }
      var DIAMETER = Number($("#DIAMETER").val());
      if (DIAMETER >= 1 && DIAMETER <= 8) {
        if(LEN >= 30 && LEN <= 170) { $("#LEN").removeClass('error'); return true; } else { $("#LEN").addClass('error'); return false; }
      } else if (DIAMETER > 8 && DIAMETER <= 12.5) {
        if(LEN >= 24 && LEN <= 64) { $("#LEN").removeClass('error'); return true; } else { $("#LEN").addClass('error'); return false; }
      } else if (DIAMETER > 12.5 && DIAMETER <= 19) {
        if(LEN >= 20 && LEN <= 60) { $("#LEN").removeClass('error'); return true; } else { $("#LEN").addClass('error'); return false; }
    ...
    function checkDiameter(compliant){
      var DIAMETER = Number($("#DIAMETER").val());
      if(compliant) {
        if(DIAMETER >= 1 && DIAMETER <= 33) { $("#DIAMETER").removeClass('error'); return true; } else { $("#DIAMETER").addClass('error'); return false; }
      } else {
        if(DIAMETER >= 1 && DIAMETER <= 33) { $("#DIAMETER").removeClass('error'); return true; } else { $("#DIAMETER").addClass('error'); return false; }
      }  
    }
    ...
    function checkDWT(compliant){
      ...
      if(compliant) {
        if(DWT >= 0.008 && DWT <= 0.01) { $("#DWT").removeClass('error'); return true; } else { $("#DWT").addClass('error'); return false; }
      } else {
        if(DWT >= 0.0015 && DWT <= 0.003) { $("#DWT").removeClass('error'); return true; } else { $("#DWT").addClass('error'); return false; }
      }  
    }
    function checkDWTTol(compliant){
      var DWT_TOL = Number($("#DWT-TOL").val());
      if(compliant) {
        if(DWT_TOL >= 0.0005) { $("#DWT-TOL").removeClass('error'); return true; } else { $("#DWT-TOL").addClass('error'); return false; }
      } else {
        if(DWT_TOL >= 0.0005) { $("#DWT-TOL").removeClass('error'); return true; } else { $("#DWT-TOL").addClass('error'); return false; }
      }  
    }
    function checkDistOd(compliant){
      ...
      // if(compliant) {
      //   if(DIST_OD >= 0.0015 && DIST_OD <= 0.003) { $("#DIST-OD").removeClass('error'); return true; } else { $("#DIST-OD").addClass('error'); return false; }
      // } else {
      //   if(DIST_OD >= 0.008 && DIST_OD <= 0.01) { $("#DIST-OD").removeClass('error'); return true; } else { $("#DIST-OD").addClass('error'); return false; }
      // }  
      return true;
    }
    function checkDistOdTol(compliant){
      ...
      if(compliant) {
        if(DIST_OD_TOL >= 0.003) { $("#DIST-OD-TOL").removeClass('error'); return true; } else { $("#DIST-OD-TOL").addClass('error'); return false; }
      } else {
        if(DIST_OD_TOL >= 0.003) { $("#DIST-OD-TOL").removeClass('error'); return true; } else { $("#DIST-OD-TOL").addClass('error'); return false; }
      }  
    }
    ...
    function checkProxOdTol(compliant){
      ...
      if(compliant) {
        if(PROX_OD_TOL >= 0.003) { $("#PROX-OD-TOL").removeClass('error'); return true; } else { $("#PROX-OD-TOL").addClass('error'); return false; }
      } else {
        if(PROX_OD_TOL >= 0.003) { $("#PROX-OD-TOL").removeClass('error'); return true; } else { $("#PROX-OD-TOL").addClass('error'); return false; }
      }  
    }
...
// function extrusionID() {
//   var DIAMETER = Number($("#DIAMETER").val());
//   var 
//   var blowRatio = 8;
//   var id = DIAMETER / blowRatio;
//   if
// }


// function checkHoop(compliant) {
//   var extrusionID = extrusionID();
//   var DIAMETER = Number($("#DIAMETER").val()); 
//   var hoopRatio = DIAMETER / extrusionID
// }

//thoughts, delete later
//How do we handle the hoop ratio situation? 
//Hoop ratio is determined by the balloon OD / extrusion ID
//And extrusion ID is balloon OD / assumed ratio of 8
//So the above would give us a minimum acceptable extrusion ID?
//How does extrusion ID affect the other tail dimensions that user inputs?
//User inputs tail OD and WT, which gives us an ID, does this have to be the ID of the extrusion?
//if so, whenever these are changed, we get an extrusion ID and we check it against the min ID that is given from the Balloon OD/ 8 assumtion

//Other things
//range of allowable wall thicknesses for tails?
//clarify DWT ranges for non and compliant
//DWT step size?
//all mm except DWT? 
...
function calculate() {
  if(['Vestamid ML21','PET','Pebax 55D', 'Pebax 63D', 'Pebax 72D'].includes($("#MATERIAL").val())) {
    ...
    var compliant = true;
      ...
      var 1 = checkDistNeck(compliant)
      var 2 = checkProxNeck(compliant)
      var 3 = checkDistNeckTol(compliant)
      var 4 = checkProxNeckTol(compliant)
      var 5 = checkLength(compliant)
      var 6 = checkLengthTol(compliant)
      var 7 =checkDiameter(compliant)
      var 8 = checkDiameterTol(compliant)
      var 9 = checkDWT(compliant)
      var 10 = checkDWTTol(compliant)
      var 11 = checkDistOd(compliant)
      var 12 = checkDistOdTol(compliant)
      var 13 = checkProxOd(compliant)
      var 14 = checkProxOdTol(compliant)
      var 15 = checkDistWT(compliant)
      var 16 = checkDistWTTol(compliant)
      var 17 = checkProxWT(compliant)
      var 18 = checkProxWTTol(compliant)
      var 19 = checkDistCone(compliant)
      var 20 = checkDistConeTol(compliant)
      var 21 = checkProxCone(compliant)
      var 22 = checkProxConeTol(compliant)
...
<div class="row">
    ...
    <div class="item"><p class="label">Diameter</p><input class="quote-input" id="l`+lineItem+`-DIAMETER" value="`+DIAMETER+`" readonly></input></div>
  	<div class="item"><p class="label">DWT</p><input class="quote-input line-length" id="l`+lineItem+`-DWT" value="`+DWT+`" readonly></input></div>
    <div class="item"><p class="label">Distal OD</p><input class="quote-input" id="l`+lineItem+`-DIST-OD" value="`+DIST_OD+`" readonly></input></div>
...
function resetInputs() {
  ...
  $("#DWT-TOL").val(0.0005);
  $("#DIST-OD-TOL, #PROX-OD-TOL").val(0.003);
  $("#DIST-WT-TOL, #PROX-WT-TOL").val(0.001);
  $("#DIST-CONE-TOL, #PROX-CONE-TOL").val(3);
  
  $("#REQUESTED-BURST, #REQUESTED-FATIGUE, #REQUESTED-VOLUME, #REQUESTED-MIN-VOLUME").val("");
...
if(['Vestamid ML21','PET','Pebax 55D', 'Pebax 63D', 'Pebax 72D'].includes($("#MATERIAL").val())) {
  ...
  $("#l"+ line + "-REQUESTED-VOLUME").attr("id",burst1ID).val($("#REQUESTED-BURST").val()).closest('p').text('Requested Burst');
  $("#l"+ line + "-REQUESTED-MIN-VOLUME").attr("id",burst2ID).val($("#REQUESTED-FATIGUE").val()).closest('p').text('Requested Fatigue');
  ...
  $("#l"+ line + "-REQUESTED-BURST").attr("id",burst1ID).val($("#REQUESTED-VOLUME").val()).closest('p').text('Requested Nominal Vol');
  $("#l"+ line + "-REQUESTED-FATIGUE").attr("id",burst2ID).val($("#REQUESTED-MIN-VOLUME").val()).closest('p').text('Requested Min Burst Vol');
  }
}
